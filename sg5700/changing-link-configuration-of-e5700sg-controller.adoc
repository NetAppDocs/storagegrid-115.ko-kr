...
.필요한 것
E5500SG 컨트롤러를 유지보수 모드로 전환해야 합니다. StorageGRID 어플라이언스를 유지보수 모드로 전환하면 원격 액세스를 위해 어플라이언스를 사용할 수 없게 될 수 있습니다.
...
+
VLAN 설정을 변경한 경우 어플라이언스의 서브넷이 변경되었을 수 있습니다. 어플라이언스의 IP 주소를 변경해야 하는 경우 IP 주소 구성 지침을 따르십시오.

+
link:setting-ip-configuration-sg5700.html["IP 구성 설정"]
