...
* * SANtricity 시스템 관리자 * 용 관리 네트워크: 이 네트워크는 E2800 컨트롤러에서 SANtricity 시스템 관리자에 대한 액세스를 제공하므로 어플라이언스의 하드웨어 구성 요소를 모니터링하고 관리할 수 있습니다. 이 관리 네트워크는 StorageGRID용 관리 네트워크와 같을 수도 있고 독립 관리 네트워크일 수도 있습니다.
