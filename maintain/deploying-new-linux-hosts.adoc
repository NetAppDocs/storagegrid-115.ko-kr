...
* 공유 스토리지를 사용하여 StorageGRID 노드를 지원하거나 의 디스크 드라이브 또는 SSD 중 일부 또는 전부를 장애 노드에서 교체 노드로 이동한 경우 원래 호스트에 있던 것과 동일한 스토리지 매핑을 다시 설정해야 합니다. 예를 들어 에서 WWID 및 별칭을 사용한 경우 `/etc/multipath.conf` 설치 지침에서 권장하는 대로 에서 동일한 별칭/WWID 쌍을 사용해야 합니다 `/etc/multipath.conf` 교체 호스트에서
