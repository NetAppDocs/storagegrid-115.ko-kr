...
+
*** * 파일 공간 이름 *: 아카이브 노드 ID와 동일합니다. 아카이브 노드 ID를 찾으려면 * 지원 * * 툴 * 그리드 토폴로지 * 를 선택합니다. 그런 다음 * Archive Node * * * ARC * Target * Overview * 를 선택합니다.
...
+
위치 `CBID` TSM 서버에서 검색할 수 없는 객체의 식별자입니다 `node_ID` 검색이 실패한 아카이브 노드의 노드 ID입니다.
...
+
하지만 개체에 대한 ILM 규칙을 사용하여 복사본을 하나만 만들고 해당 복사본이 손실되면 개체를 복구할 수 없습니다. 이 경우 를 실행합니다 `Object_Lost` 명령은 StorageGRID 시스템에서 손실된 개체의 메타데이터를 지웁니다.
...
+
.. Archive Node * * * ARC * * Retrieve * * Configuration * 로 이동하여 * Reset Request Failure Count * 를 선택합니다.
.. 변경 내용 적용 * 을 클릭합니다.
