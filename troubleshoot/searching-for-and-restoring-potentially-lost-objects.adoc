...
.. grep를 사용하여 잠재적으로 손실된 개체와 관련된 감사 메시지를 추출하고 출력 파일로 보냅니다. 입력: `grep uuid-valueaudit_file_name > output_file_name`
+
예를 들면 다음과 같습니다.
...
link:troubleshooting-storagegrid-system.html["객체 데이터 위치를 확인하는 중입니다"]
