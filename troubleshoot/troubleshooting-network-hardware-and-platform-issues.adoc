...
** Linux 및 VMware 기반 노드의 경우 다음 명령을 사용합니다. `+/usr/sbin/change-mtu.py [-h] [-n node] mtu network [network...]+`
+
* 예 *: `change-mtu.py -n node 1500 grid admin`

+
* 참고 *: Linux 기반 노드에서 컨테이너에 있는 네트워크에 대해 원하는 MTU 값이 호스트 인터페이스에 이미 구성된 값을 초과하는 경우, 먼저 원하는 MTU 값을 사용하도록 호스트 인터페이스를 구성한 다음 를 사용해야 합니다 `change-mtu.py` 컨테이너에서 네트워크의 MTU 값을 변경하는 스크립트입니다.
...
.관련 정보
link:troubleshooting-storagegrid-system.html["Grid Network MTU 불일치 알림 문제 해결"]
...
----
. Linux에서 Docker 컨테이너 및 제어 StorageGrid 노드 프로세스를 중지합니다.``sudo docker stop --time secondscontainer-name``
+
...
+
image::../media/node_overview_ip_addresses_ipv6.gif[노드 스크린샷 > 개요 > IP 주소]
