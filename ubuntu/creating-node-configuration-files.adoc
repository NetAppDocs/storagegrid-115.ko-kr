...
의 모든 파일 `/etc/storagegrid/nodes` 이러한 명명 규칙을 따르지 않는 명명 규칙은 호스트 서비스에서 구문 분석되지 않습니다.
...
""그리드 노드가 기본 관리 노드를 검색하는 방법"을 참조하십시오.
...
MAC 복제에 대한 자세한 내용은 MAC 주소 복제에 대한 고려 사항 및 권장 사항을 참조하십시오.
...
block_device_RANGEDB_00만 필요하며 나머지는 선택 사항입니다. BLOCK_DEVICE_RANGEDB_00에 지정된 블록 디바이스는 4TB 이상이어야 하며 다른 블록 디바이스는 더 작을 수 있습니다.

* 참고 *: 틈을 남기지 마십시오. BLOCK_DEVICE_RANGEDB_05를 지정하는 경우 BLOCK_DEVICE_RANGEDB_04도 지정해야 합니다.
...
* 참고 *: RAM 값은 노드의 실제 메타데이터 예약 공간에 영향을 줍니다. 메타데이터 예약된 공간에 대한 설명은 StorageGRID 관리 지침을 참조하십시오.
...
사용되는 형식은 다음과 같습니다. `<network type>/<protocol>/<default port used by grid node>/<new port>`네트워크 유형이 grid, admin 또는 client이고 프로토콜은 TCP 또는 UDP입니다.
...
사용되는 형식은 다음과 같습니다. `<network type>/<protocol:>/<remapped port >/<default port used by grid node>`네트워크 유형이 grid, admin 또는 client이고 프로토콜은 TCP 또는 UDP입니다.
