...
NOTE: 포트 연결 테이블에 나열된 그리드 네트워크 포트는 StorageGRID 버전 11.5.0에만 유효합니다. 각 노드 유형에 맞는 포트를 확인하려면 사용 중인 StorageGRID 버전에 대한 네트워킹 지침을 항상 참조해야 합니다.
...
+
image::../media/nmap_test_table.png[포트 연결 테이블]
